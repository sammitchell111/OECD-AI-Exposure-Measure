#!/usr/bin/env python3
"""
Occupational AI Capability Rating Pipeline — 10 Occupations
============================================================
Identical to run_ratings.py but operates on a smaller set of 10 occupations.
...
Features:
  ...
  - Outputs to 10_occupations_ratings/<model_folder>/ with files named
    <Model>_run_N_10.json / .xlsx

Usage:
    python run_ratings_10.py
"""
...
# Configuration
# ---------------------------------------------------------------------------
INPUT_JSON_FILE = "10_additional_occupations.json"
...
OUTPUT_ROOT = os.path.join(RESULTS_DIR, "10_occupations_ratings")
...
def find_incomplete_run(folder: str, total_occupations: int, file_prefix: str) -> Optional[int]:
    ...
    for name in sorted(os.listdir(folder)):
        if name.startswith(f"{file_prefix}_run_") and name.endswith("_10.json"):
            try:
                # Extract run number from e.g. "Claude_run_1_10.json"
                parts = name.replace(f"{file_prefix}_run_", "").replace("_10.json", "")
                num = int(parts)
...
def next_run_number(folder: str, file_prefix: str) -> int:
    ...
    for name in os.listdir(folder):
        if name.startswith(f"{file_prefix}_run_") and "_10." in name:
            try:
                # Extract run number from e.g. "Claude_run_3_10.json"
                parts = name.replace(f"{file_prefix}_run_", "").split("_10.")[0]
                num = int(parts)
                existing.append(num)
...
def main():
    print("=" * 60)
    print("  Occupational AI Capability Rating Pipeline — 10 Occupations")
    print("=" * 60)
    ...
    if incomplete is not None:
        run_num = incomplete
        print(f"\n  Found incomplete {file_prefix}_run_{run_num}_10 — resuming it.")
    else:
        run_num = next_run_number(folder_name, file_prefix)

    excel_path = os.path.join(folder_name, f"{file_prefix}_run_{run_num}_10.xlsx")
    json_path = os.path.join(folder_name, f"{file_prefix}_run_{run_num}_10.json")

    print(f"  Output folder: {folder_name}/")
    print(f"  Run number:    {run_num}")
    print(f"  Files:         {file_prefix}_run_{run_num}_10.json / .xlsx")
