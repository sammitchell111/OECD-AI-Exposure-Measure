#!/usr/bin/env python3
"""
IWA AI Capability Rating Pipeline
=================================
Sends each Intermediate Work Activity (IWA) to a selected LLM for rating
across 9 AI capability scales.
...
Features:
  ...
  - Resume-from-checkpoint: re-running skips already-rated IWAs
  - Uses auxiliary information per IWA (linked DWAs, tasks, occupations)

Usage:
    python run_ratings_IWA.py
"""
...
PROMPT_FILE = "rating_instructions_iwa.txt"
...
# Maximum retries per IWA on transient errors
MAX_RETRIES = 3
...
def create_rating_prompt(
    ...
    iwa_name = iwa_data.get("iwa_name", "Unknown")
    gwa_code = iwa_data.get("gwa_code", "")
    gwa_name = iwa_data.get("gwa_name", "")
    reference = iwa_data.get("reference", [])
    linked_dwas = iwa_data.get("linked_dwas", [])
    linked_tasks = iwa_data.get("linked_tasks", [])
    linked_occupations = iwa_data.get("linked_occupations", [])
    ...
    # Format linked DWAs (always all of them — IWAs have at most a few dozen DWAs)
    if linked_dwas:
        linked_dwas_text = (
            f"\nLINKED DETAILED WORK ACTIVITIES (DWAs) — {len(linked_dwas)} total:\n"
        )
        for i, dwa in enumerate(linked_dwas, 1):
            code = dwa.get("code", "")
            name = dwa.get("name", "")
            linked_dwas_text += f"{i}. [{code}] {name}\n"
    else:
        linked_dwas_text = "\n(No linked DWAs available for this IWA)\n"

    # Format linked tasks (all of them per user instruction)
    if linked_tasks:
        linked_tasks_text = (
            f"\nLINKED O*NET TASKS — {len(linked_tasks)} total:\n"
        )
        for i, task in enumerate(linked_tasks, 1):
            tid = task.get("task_id", "")
            ttext = task.get("task", "")
            linked_tasks_text += f"{i}. [Task {tid}] {ttext}\n"
    else:
        linked_tasks_text = "\n(No linked tasks available for this IWA)\n"

    # Format linked occupations (all of them per user instruction)
    if linked_occupations:
        linked_occupations_text = (
            f"\nLINKED OCCUPATIONS — {len(linked_occupations)} total:\n"
        )
        for i, occ in enumerate(linked_occupations, 1):
            code = occ.get("code", "")
            title = occ.get("title", "")
            linked_occupations_text += f"{i}. [{code}] {title}\n"
    else:
        linked_occupations_text = "\n(No linked occupations available for this IWA)\n"
    ...
    return prompt_template.format(
        ...
        linked_dwas_text=linked_dwas_text,
        linked_tasks_text=linked_tasks_text,
        linked_occupations_text=linked_occupations_text,
...
def result_to_row(result: Dict) -> Dict:
    ...
    row = {
        "IWA Code": result["iwa_code"],
        "IWA Name": result["iwa_name"],
        "GWA Code": result.get("gwa_code", ""),
        "GWA Name": result.get("gwa_name", ""),
        "Reference": result.get("reference", ""),
        "Number of Linked DWAs": result.get("num_dwas", 0),
        "Number of Linked Tasks": result.get("num_tasks", 0),
        "Number of Linked Occupations": result.get("num_occupations", 0),
    ...
    for scale_name in SCALE_NAMES:
        ...
        # NOTE: "Level Description" columns are intentionally omitted (same as original pipeline)
    return row
...
def save_excel(rows: List[Dict], excel_path: str):
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        df.to_excel(writer, sheet_name="IWA Ratings", index=False)
        ws = writer.sheets["IWA Ratings"]
...
async def process_iwa(
    ...
    """Process a single IWA with concurrency control."""
    async with sem:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                print(f"  [{idx}/{total}] {data.get('iwa_name', code)}")
                ref = data.get("reference", [])
                if isinstance(ref, list):
                    ref = "; ".join(ref)
                return {
                    "iwa_code": code,
                    "iwa_name": data.get("iwa_name"),
                    "gwa_code": data.get("gwa_code"),
                    "gwa_name": data.get("gwa_name"),
                    "reference": ref,
                    "num_dwas": len(data.get("linked_dwas", [])),
                    "num_tasks": len(data.get("linked_tasks", [])),
                    "num_occupations": len(data.get("linked_occupations", [])),
                ...
                wait = 2 ** attempt
                print(
                    f"  [{idx}/{total}] {data.get('iwa_name', code)} — "
                    f"attempt {attempt} failed ({e}), retrying in {wait}s…"
                )
                await asyncio.sleep(wait)

        # All retries exhausted
        print(f"  [{idx}/{total}] {data.get('iwa_name', code)} — FAILED after {MAX_RETRIES} attempts")
        ref = data.get("reference", [])
        if isinstance(ref, list):
            ref = "; ".join(ref)
        return {
...
async def run_pipeline(
    ...
    # Filter out already-processed IWAs (resume support)
    to_process = {
        code: data
        for code, data in iwas.items()
        if code not in existing_codes
    }

    if len(to_process) < len(iwas):
        print(
            f"Resuming: {len(iwas) - len(to_process)} already processed, "
            f"{len(to_process)} remaining.\n"
        )

    if not to_process:
        print("All IWAs already processed. Nothing to do.")
        return

    # Load existing *successful* results into a dict keyed by IWA code.
    # This ensures only one entry per IWA and drops old error entries.
    ...
    print(f"Processing {total} IWAs with concurrency={MAX_CONCURRENT}…\n")
    ...
    for batch_start in range(0, total, MAX_CONCURRENT):
        ...
        for result in results:
            if "error" not in result:
                results_by_code[result["iwa_code"]] = result
...
# Friendly folder names for each provider
...
def find_incomplete_run(folder: str, total_iwas: int) -> Optional[int]:
    """Check if any existing run is incomplete (fewer results than total IWAs)."""
...
def main():
    print("=" * 60)
    print("  IWA AI Capability Rating Pipeline")
    print("=" * 60)
    ...
    with open(INPUT_JSON_FILE, "r", encoding="utf-8") as f:
        iwas = json.load(f)
    print(f"  {len(iwas)} IWAs loaded from {INPUT_JSON_FILE}")
    ...
    incomplete = find_incomplete_run(folder_name, len(iwas))
    ...
    if os.path.exists(json_path):
        with open(json_path, "r", encoding="utf-8") as f:
            prior = json.load(f)
            existing_codes = {r["iwa_code"] for r in prior if "error" not in r}
        print(f"  Found checkpoint: {len(existing_codes)} IWAs already done")
    ...
    asyncio.run(
        run_pipeline(
            ...
            iwas, scales_info, prompt_template,
    ...
    print("=" * 60)
    print(f"  Complete!  {len(iwas)} IWAs  |  {mins}m {secs}s elapsed")
