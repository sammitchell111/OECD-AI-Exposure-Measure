#!/usr/bin/env python3
"""
Occupational AI Capability Rating Pipeline (Lightweight Models)
================================================================
Sends each occupation to a selected lightweight LLM for rating across 9 AI capability scales.

Features:
  - Interactive LLM selection at startup (lightweight models from Anthropic / OpenAI / Google / Mistral)
  ...
  - Model name included in output file names

Usage:
    python run_ratings_lightweight.py
"""
...
# ---------------------------------------------------------------------------
# Prompt construction (identical logic to notebook)
# ---------------------------------------------------------------------------
...
def select_provider():
    ...
    print("\nSelect which LLM provider to use (LIGHTWEIGHT models):")
    print("  1. Anthropic  (Claude Haiku 4.5)")
    print("  2. OpenAI     (GPT-5.4 Mini)")
    print("  3. Google     (Gemini 3.1 Flash-Lite Preview)")
    print("  4. Mistral    (Mistral Small 4)")
    print()
    ...
    if choice == "1":
        ...
        print("\n--- Selected: ANTHROPIC (Lightweight) ---")
        api_key = load_api_key("ANTHROPIC_API_KEY.txt") or getpass.getpass(
            "Enter your Anthropic API Key: "
        )
        client = Anthropic(api_key=api_key)
        print("Ready: ANTHROPIC (claude-haiku-4-5-20251001)\n")
        return "anthropic", "claude-haiku-4-5-20251001", client

    elif choice == "2":
        from openai import OpenAI

        print("\n--- Selected: OPENAI (Lightweight) ---")
        api_key = load_api_key("OPENAI_API_KEY.txt") or getpass.getpass(
            "Enter your OpenAI API Key: "
        )
        client = OpenAI(api_key=api_key)
        print("Ready: OPENAI (gpt-5.4-mini-2026-03-17)\n")
        return "openai", "gpt-5.4-mini-2026-03-17", client

    elif choice == "3":
        import google.generativeai as genai

        print("\n--- Selected: GOOGLE (Lightweight) ---")
        ...
        print("Ready: GOOGLE (gemini-3.1-flash-lite-preview)\n")
        return "google", "gemini-3.1-flash-lite-preview", client
        ...
        print("\n--- Selected: MISTRAL (Lightweight) ---")
        api_key = load_api_key("MISTRAL_API_KEY.txt") or getpass.getpass(
            "Enter your Mistral API Key: "
        )
        client = Mistral(api_key=api_key)
        print("Ready: MISTRAL (mistral-small-2603)\n")
        return "mistral", "mistral-small-2603", client
...
def result_to_row(result: Dict) -> Dict:
    ...
    for scale_name in SCALE_NAMES:
        ...
        # NOTE: "Level Description" columns are intentionally omitted (user request #2)
    return row
...
# Friendly folder names for each provider (lightweight model variants)
PROVIDER_FOLDER_NAMES = {
    "anthropic": "claude_haiku",
    "openai": "chatgpt_mini",
    "google": "gemini_flash_lite",
    "mistral": "mistral_small",
}


def _make_safe_model_tag(model_name: str) -> str:
    """Convert a model name to a filesystem-safe tag for use in filenames."""
    return model_name.replace("/", "-").replace(" ", "-")


def find_incomplete_run(folder: str, model_tag: str, total_occupations: int) -> Optional[int]:
    """Check if any existing run is incomplete (fewer results than total occupations)."""
    if not os.path.exists(folder):
        return None
    prefix = f"{model_tag}_run_"
    for name in sorted(os.listdir(folder)):
        if name.startswith(prefix) and name.endswith(".json"):
            try:
                num = int(name.replace(prefix, "").split(".")[0])
...
def next_run_number(folder: str, model_tag: str) -> int:
...
def main():
    print("=" * 60)
    print("  Occupational AI Capability Rating Pipeline (Lightweight)")
    print("=" * 60)
    ...
    # Create a filesystem-safe model tag for use in filenames
    model_tag = _make_safe_model_tag(model_name)

    # Check for an incomplete run first; if found, resume it
    incomplete = find_incomplete_run(folder_name, model_tag, len(occupations))
    if incomplete is not None:
        run_num = incomplete
        print(f"\n  Found incomplete {model_tag}_run_{run_num} — resuming it.")
    else:
        run_num = next_run_number(folder_name, model_tag)

    excel_path = os.path.join(folder_name, f"{model_tag}_run_{run_num}.xlsx")
    json_path = os.path.join(folder_name, f"{model_tag}_run_{run_num}.json")
